...
const AgentCard = ({ agent, status, output, isActive, onViewOutput, index, totalAgents }) => {
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <div className={`${agent.bgColor} p-3 rounded-lg`}>
              {agent.icon && React.createElement(agent.icon, { className: 'w-6 h-6 text-white' })}
            </div>
            <div>
          ...
          <div className="flex items-center space-x-2">
            <StatusIcon className={`w-5 h-5 ${statusConfig.statusColor} ${status === 'working' ? 'animate-spin' : ''}`} />
            <span className={`text-sm font-medium ${statusConfig.statusColor}`}>{statusConfig.statusText}</span>
          </div>
        </div>
        ...
        <div className="bg-gray-900/50 rounded-lg p-4 mb-4 min-h-[80px]">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-gray-400 uppercase tracking-wide">Output Preview</span>
            {output && <span className="text-xs text-gray-500">{output.length} chars</span>}
          </div>
          ...
          <div className="text-sm text-gray-300 leading-relaxed">
            {status === 'working' ? (
              ...
              <p className="text-gray-400">{getOutputPreview()}</p>
            )}
          </div>
        </div>
          ...
          <div className="flex items-center space-x-2">
            <div
              className={`w-2 h-2 rounded-full ${
                status === 'completed' ? 'bg-green-400' : status === 'working' ? 'bg-blue-400' : status === 'error' ? 'bg-red-400' : 'bg-gray-500'
              }`}
            />
            <span className="text-xs text-gray-400">Step {index + 1} of {totalAgents}</span>
          </div>

          {output && status === 'completed' && (
            <button
              onClick={() => onViewOutput(agent, output)}
              className="flex items-center space-x-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg transition-colors duration-200"
            >
